...
/*   Updated: 2024/12/25 17:48:14 by yohatana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_charactaer_check(t_map *map)
{
	...
	while (k < map->width)
	{
		route_map[i] = (int *)ft_calloc(sizeof(int), map->width);
		if (!route_map[i])
			return (0);
		k++;
	}
...
// void    move(int x, int y, t_map *data)
// {
// 	if (x < 0 || data->map_x <= x || y < 0 || data->map_y <= y || data->map[y][x] != 0)
// 		return ;
// 	data->map[y][x] = 5;
// 	move(x - 1, y, data);
// 	move(x + 1, y, data);
